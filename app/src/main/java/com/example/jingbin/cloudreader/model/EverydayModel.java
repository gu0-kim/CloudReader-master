...
public class EverydayModel {
    ...
    public void showBanncerPage(final RequestImpl listener) {
        Subscription subscription = HttpUtils.getInstance().getDongTingServer().getFrontpage().observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribe(new Observer<FrontpageBean>() {
            @Override
    ...
    public void showRecyclerViewData(final RequestImpl listener) {
        ...
        Subscription subscription = HttpUtils.getInstance().getGankIOServer().getGankIoDay(year, month, day).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).flatMap(func1).subscribe(observer);
        listener.addSubscription(subscription);
    }
